...
export class DesignToSpecConverter {
    fs;
    resolver;
    constructor(fs, resolver) {
        this.fs = fs;
        this.resolver = resolver;
    }
    async convert(changeName) {
        const result = {
            success: false,
            specs: [],
            warnings: [],
    ...
    extractCapabilities(designContent) {
        const capabilities = [];
    ...
    createDefaultCapability(designContent, changeName) {
    ...
    extractKeySections(designContent) {
        const sections = [];
    ...
    generateSpecContent(capability, changeName) {
        const lines = [];
        lines.push(`---`);
    ...
    async validateConversion(changeName) {
        const changeDir = this.resolver.changeDir(changeName);
        const specsDir = path.join(changeDir, 'specs');
...
//# sourceMappingURL=design-to-spec-converter.js.map