...
async function copyFileWithPolicy(src, dest, options) {
...
export async function syncDrivAssets(baseDir, options = {}) {
    ...
    const configResult = await copyFileWithPolicy(sourceConfig, path.join(targetDrivDir, 'config.yaml'), options);
