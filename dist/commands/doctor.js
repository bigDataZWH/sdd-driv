...
function getScopeBases(projectPath, scope) {
    if (scope === 'project')
        return [{ scope: 'project', baseDir: projectPath }];
    if (scope === 'global')
        return [{ scope: 'global', baseDir: os.homedir() }];
    const bases = [
...
async function checkSkillCompleteness(projectPath, scope) {
    ...
    if (!anyFound) {
        results.push({
            ...
            message: scope === 'global' ? 'no skills found in global scope' : 'no skills found — run: driv init',
        });
    }
    return results;
}
async function checkDrivYamlValidity(projectPath) {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            results.push(unknownFields.length === 0
                ? { check: `.driv.yaml: ${entry}`, status: 'pass', message: 'valid' }
                : {
