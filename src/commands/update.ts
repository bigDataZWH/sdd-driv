...
export function getDrivSkills(): string[] {
  return [...DRIV_SKILLS];
}

type SkillLanguage = 'en' | 'zh';

interface InstalledDrivTarget {
  scope: InstallScope;
  platform: Platform;
...
function getInstalledDrivSkillsDirs(
  baseDir: string,
  platform: Platform,
...
async function hasLocalDrivSkills(
  baseDir: string,
  platform: Platform,
  scope: InstallScope,
): Promise<boolean> {
  for (const skillsDir of getInstalledDrivSkillsDirs(baseDir, platform, scope)) {
    ...
    if (entries.some((entry) => entry.startsWith('driv'))) return true;
  }
  return false;
}

async function detectInstalledDrivLanguage(
  baseDir: string,
  platform: Platform,
  scope: InstallScope = 'project',
...
async function detectInstalledDrivTargets(
  projectPath: string,
  options: { scopes?: InstallScope[]; globalBaseDir?: string } = {},
): Promise<InstalledDrivTarget[]> {
  const scopes = options.scopes ?? (['project', 'global'] as InstallScope[]);
  const targets: InstalledDrivTarget[] = [];

  for (const scope of scopes) {
    const baseDir = getScopedBaseDir(scope, projectPath, options.globalBaseDir);

    for (const platform of PLATFORMS) {
      if (!(await hasLocalDrivSkills(baseDir, platform, scope))) continue;
...
async function detectDrivPackageScope(
  projectPath: string,
...
async function updateDrivNpmPackage(
  scope: InstallScope,
  projectPath: string,
...
export async function updateCommand(
  ...
  if (!options.skipNpm) {
    ...
    const npmUpdated = await updateDrivNpmPackage(
      packageScope,
      projectPath,
      log,
      options.json === true,
  ...
  for (const target of targets) {
    ...
    const templates =
      target.scope === 'project' ? await syncDrivAssets(baseDir, { overwrite }) : { copied: 0, skipped: 0 };
    ...
    log(
      `  ${target.platform.name} (${target.scope}, ${languageSkillsDir}): ${commands.copied} copied, ${commands.skipped} skipped`,
    );
  }
